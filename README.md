# Semester V — Field Journal

A static "academic diary" frontend for [`hamnasz/Semester-V`](https://github.com/hamnasz/Semester-V) —
...
`Semester-V`, then renders it. Push a new file to that repo and it shows up here the next
...
1. **On load**, `js/app.js` calls `GitHubAPI.fetchTree()`, which hits
   `GET /repos/hamnasz/Semester-V/git/trees/main?recursive=1` — one API call returns the
...
3. **File previews** are fetched on demand, straight from
   `raw.githubusercontent.com/hamnasz/Semester-V/main/<path>`, only when you open a file —
...
Matching is case-insensitive and trims whitespace. If a new subject folder is added to the
repo, add its name here too, or it won't appear in the journal.
...
```js
const CONFIG = { owner: 'hamnasz', repo: 'Semester-V', branch: 'main' };
```
...
4. That's the entire deployment. There's no manifest to regenerate afterwards — new files
   pushed to `hamnasz/Semester-V` just appear the next time the journal is opened.
